...
// Specular:
            ...
            float3 diffuseColor = Function_node_4182( Function_node_5074( normalize(mul( _World2Object, float4(viewDirection,0) ).xyz.rgb) , mul( _World2Object, float4(i.posWorld.rgb,0) ).xyz.rgb ) ); // Need this for specular when using metallic
...
/// Vectors:
            ...
            float3 diffColor = Function_node_4182( Function_node_5074( normalize(mul( _World2Object, float4(viewDirection,0) ).xyz.rgb) , mul( _World2Object, float4(i.posWorld.rgb,0) ).xyz.rgb ) );
            float specularMonochrome;
            float3 specColor;
